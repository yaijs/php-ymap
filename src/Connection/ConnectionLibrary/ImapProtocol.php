...
use Yai\Ymap\Connection\ConnectionLibrary\Exceptions\ResponseException;
...
class ImapProtocol extends Protocol {
     ...
     * @throws RuntimeException
     */
    protected function nextTaggedLineIgnoreUntagged(Response $response, ?string &$tag): string {
        do {
            $line = $this->nextLine($response);
     ...
     * @throws RuntimeException
     */
    protected function assumedNextTaggedLineIgnoreUntagged(Response $response, string $start, ?string &$tag): bool {
    ...
    protected function decodeLine(Response $response, string $line): array {
        ...
        while (($pos = strpos($line, ' ')) !== false) {
            ...
            if ($token[0] == '{') {
                ...
                if (is_numeric($chars)) {
                    $literalLength = (int)$chars;
                    $token = '';
                    while (strlen($token) < $literalLength) {
                        $token .= $this->nextLine($response);
                    }
                    $line = '';
                    if (strlen($token) > $literalLength) {
                        $line = substr($token, $literalLength);
                        $token = substr($token, 0, $literalLength);
                    } else {
                        $line .= $this->nextLine($response);
                    }
                    $tokens[] = $token;
     ...
     * @param Response $response
     * @param array<int, mixed>|string $tokens to decode
     ...
     * @param array<int, mixed> $arr array to stringify
     *
     ...
     * @param array<int, mixed> $tokens additional parameters to command, use escapeString() to prepare
     ...
     *
     * @param string $command
     * @param array<int, mixed> $tokens parameters as in sendRequest()
     ...
     * @param array<int, string>|string $string the literal/-s
     *
     * @return string|array<int, mixed> escape literals, literals with newline ar returned
     *                      as array('{size}', 'string');
     */
    public function escapeString(array|string $string): array|string {
        if (func_num_args() < 2) {
            if (is_array($string)) { // Handle array passed directly
                 $result = [];
                 foreach($string as $s) {
                     $result[] = $this->escapeString($s);
                 }
                 return $result;
            }
     ...
     * @param array<int, mixed> $list list with literals or lists as PHP array
     *
     ...
     * @param array<int, string>|string $items items to fetch [RFC822.HEADER, FLAGS, RFC822.TEXT, etc]
     * @param array<int, int|string>|int|string $from message for items or start message if $to !== null
     ...
     * @throws RuntimeException
     */
    public function fetch(array|string $items, array|int|string $from, int|float|null $to = null, int|string $uid = IMAP::ST_UID): Response {
        if (is_array($from)) {
            if (count($from) === 0) {
                $set = '';
            } elseif (count($from) > 1) {
                $set = implode(',', array_map(static fn (int|string $value): string => (string)$value, $from));
            } else {
                $fromValues = array_values($from);
                $first = (string)$fromValues[0];
                $set = $first . ':' . $first;
            }
        } else {
            if ($to === null) {
        ...
        while (!$this->readLine($response, $tokens, $tag)) {
            ...
            if ($to === null && !is_array($from)) {
                if ($uid === IMAP::ST_UID) {
                    if ($uidKey !== null && (($tokens[2][$uidKey] ?? null) != $from)) {
                        continue;
                    }
                } elseif ($tokens[0] != $from) {
                    continue;
                }
            }
            ...
            if (count($items) == 1) {
                // Normalize item name for matching: server drops .PEEK from responses
                // e.g. BODY.PEEK[1] is returned as BODY[1] per RFC 3501
                $matchItem = str_ireplace('.PEEK', '', $items[0]);

                if (isset($tokens[2][0]) && strcasecmp($tokens[2][0], $matchItem) === 0) {
                    $data = $tokens[2][1];
                } elseif ($uid === IMAP::ST_UID && isset($tokens[2][2]) && strcasecmp($tokens[2][2], $matchItem) === 0) {
                    $data = $tokens[2][3];
                } else {
                    $expectedResponse = 0;
                    ...
                    for ($i = 2; $i < $count; $i += 2) {
                        if (strcasecmp($tokens[2][$i], $matchItem) !== 0) {
                            continue;
                        }
                        $data = $tokens[2][$i + 1];
                        $expectedResponse = 1;
                ...
                while (key($tokens[2]) !== null) {
                    $data[strtoupper(current($tokens[2]))] = next($tokens[2]);
                    next($tokens[2]);
                }
            }
            ...
            if ($to === null && !is_array($from)) {
                if ($uid === IMAP::ST_UID) {
                    // Some servers omit UID in FETCH response attributes; in that case
                    // we already filtered by request scope and can accept this row.
                    $isRequestedMessage = $uidKey === null || (($tokens[2][$uidKey] ?? null) == $from);
                } else {
                    $isRequestedMessage = $tokens[0] == $from;
                }
            }
            if ($isRequestedMessage) {
                // we still need to read all lines
                $dummy = [];
                while (!$this->readLine($response, $dummy, $tag));
                return $response->setResult($data);
            }
            if ($uid === IMAP::ST_UID) {
                $resultKey = $uidKey !== null ? ($tokens[2][$uidKey] ?? $tokens[0]) : $tokens[0];
        ...
        if ($to === null && !is_array($from)) {
            ...
            // Allow empty result without throwing, consistent with other methods
             return $response->setResult(null)->setCanBeEmpty(true);
        }
    ...
    public function folders(string $reference = '', string $folder = '*'): Response {
        $tokens = $this->escapeString($reference, $folder);
        $tokens = is_array($tokens) ? $tokens : [$tokens];
        $response = $this->requestAndResponse('LIST', $tokens)->setCanBeEmpty(true);
        $list = $response->data();

        $result = [];
        if (is_array($list) && isset($list[0]) && $list[0] !== true) {
            foreach ($list as $item) {
                if (!is_array($item) || count($item) != 4 || $item[0] != 'LIST' || !is_string($item[3])) {
                    continue;
                }
     ...
     * Manage flags
     *
     * @param array<int, string>|string $flags flags to set, add or remove - see $mode
    ...
    public function store(
        ...
        $result = [];
        foreach ((array)$response->data() as $token) {
            if (!is_array($token) || ($token[1] ?? null) != 'FETCH' || !isset($token[2][0]) || $token[2][0] != 'FLAGS') {
                continue;
            }
     ...
     *
     * @param array<int, string> $messages List of message identifiers
     * @param string $folder Destination folder
     * @param int|string $uid set to IMAP::ST_UID or any string representing the UID - set to IMAP::ST_MSGN to use
     *                        message numbers instead.
     *
    ...
    public function moveMessage(string $folder, int|string $from, ?int $to = null, int|string $uid = IMAP::ST_UID): Response {
        ...
        // - https://github.com/ConnectionLibrary/php-imap/issues/123
        if (!$result->boolean()) {
            $result = $this->copyMessage($folder, $from, $to, $uid);
            if (!$result->boolean()) {
                return $result;
            }
            $result = $this->store(['\Deleted'], (int)$from, $to, null, true, $uid);
            if (!$result->boolean()) {
                return $result;
            }
            return $this->expunge();
    ...
    public function moveManyMessages(array $messages, string $folder, int|string $uid = IMAP::ST_UID): Response {
        ...
        if (!$result->boolean()) {
            ...
            foreach ($messages as $message) {
                ...
                $result = $this->store(['\Deleted'], $messageId, $messageId, null, true, $uid);
                if (!$result->boolean()) {
                    return $result;
                }
            }
    ...
    public function renameFolder(string $old, string $new): Response {
        $tokens = $this->escapeString($old, $new);
        $tokens = is_array($tokens) ? $tokens : [$tokens];
        return $this->requestAndResponse('RENAME', $tokens, true);
    }
    ...
    public function search(array $params, int|string $uid = IMAP::ST_UID): Response {
        ...
        foreach ((array)$response->data() as $ids) {
            if (is_array($ids) && isset($ids[0]) && $ids[0] === 'SEARCH') {
                array_shift($ids);
    ...
    public function overview(string $sequence, int|string $uid = IMAP::ST_UID): Response {
        ...
        if (!empty($ids)) {
            ...
            $response->stack($flags);

            $flagsData = $flags->data();
            $flagsById = is_array($flagsData) ? $flagsData : [];

            foreach ($headers->data() as $id => $raw_header) {
                $attributes = (new Header($raw_header, $this->config))->getAttributes();
                $rowFlags = $flagsById[$id] ?? null;
                if (is_string($rowFlags)) {
                    $rowFlags = [$rowFlags];
                }
                if (is_array($rowFlags)) {
                    $attributes['flags'] = $rowFlags;
                }
     ...
     * @return string
     */
    public function buildSet(int|string $from, int|float|null $to = null): string {
        $set = (string)(int)$from;
        if ($to !== null) {
            $set .= ':' . ($to == INF ? '*' : (int)$to);
        }
